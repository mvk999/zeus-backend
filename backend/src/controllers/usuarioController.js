...
class UsuarioController {
  ...
  static listar(req, res) {
    DAOusuario.listarTodos((err, usuarios) => {
      if (err) {
        return res.status(500).json({ erro: 'Erro ao listar usuários', err: err.message });
      }
      res.json(usuarios);
    });
  }
  ...
  static login(req, res) {
    ...
    DAOusuario.buscarPorEmail(email_user, async (err, usuario) => {
      if (err) {
        return res.status(500).json({ erro: 'Erro interno ao buscar usuário.', err: err.message });
      }
...
static esquecisenha(req, res) {
  ...
  if (err) {
    ...
    return res.status(500).json({ erro: 'Erro ao enviar o código por email.', err: err.message });
  }
...
static redefinirSenha(req, res) {
  ...
  static deletar(req, res) {
    ...
    DAOusuario.deletar(id, (err, resultado) => {
      if (err) {
        return res.status(500).json({ erro: 'Erro ao deletar usuário', err: err.message });
      }
